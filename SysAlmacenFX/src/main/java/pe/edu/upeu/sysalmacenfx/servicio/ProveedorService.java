...
public class ProveedorService {
    ...
    //U
    public Proveedor update(Cliente to, Long id) {
        try {
            Proveedor toe=repo.findById(id).get();
            if(toe!=null) {
                toe.setNombresRaso(to.getNombres());
            }
    ...
    public List<ComboBoxOption> listarCombobox(){
        ...
        for (Proveedor cate : repo.findAll()) {
            listar.add(new ComboBoxOption(String.valueOf(cate.getIdProveedor()),cate.getNombresRaso()));
        }
        return listar;
